...
class KNNClassifier(ClassifierMixin, BaseEstimator):
    def __init__(self, n_neighbors=5, use_idf=True, analyzer=stemmed_words, chi_portion=0.5):
...
if __name__ == "__main__":
    ...
    if not LOAD_MODEL:
        ...
        clf = GridSearchCV(estimator=KNNClassifier(), param_grid=param_grid, scoring='f1', n_jobs=5, verbose=3, error_score='raise')
        clf.fit(X_train, y_train)
        # Save to disk
        model = clf.best_estimator_
        print(f"\n=== Best model's params:\n {clf.best_params_}\n")
        joblib.dump(model, 'knn_classifier.sav')
    else:
